...
// ¿No sería mejor hacer un solo "echo" y poner todo entre comillas?
...
echo "<a href='insert.php'>Agregar Nuevo</a><br><br>"; //br br mala práctica

if ($result->num_rows > 0) {
    echo "<table border='1' cellpadding='10'>";  //esto está mal? deberia hacerse en css no?
    echo "<tr><th>Nombre</th><th>Email</th><th>Edad</th><th>Acciones</th></tr>";
    while ($row = $result->fetch_assoc()) { //obtiene una fila de resultado "como un array asociativo"
        echo "<tr>
                <td>{$row['fullname']}</td>
                <td>{$row['email']}</td>
                <td>{$row['age']}</td>
              ...
              </tr>"; //nombres en ingles de la base de datos, muestra al cliente en español
    }
    echo "</table>";
} else {
